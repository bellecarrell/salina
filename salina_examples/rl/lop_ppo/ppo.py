...
def run_lop(ppo_action_agent, ppo_critic_agent, logger, cfg):
    ...
    for epoch in range(cfg.algorithm.max_epochs):
        ...
        for _ in range(cfg.algorithm.pi_epochs):
            ...
            # begin modified for LOP
            ppo_action_agent.phi1.parameters
            phi1_params = parameters_to_vector(ppo_action_agent.phi1.parameters())
            phi2_params = parameters_to_vector(ppo_action_agent.phi2.parameters())

            cosine_sim = nn.CosineSimilarity(dim=0)
            cosine_sim = cosine_sim(phi1_params, phi2_params)
            loss_pi = loss_pi - cfg.algorithm.aux_weight*cosine_sim
...
def main(cfg):
    ...
    run_lop(action_agent, critic_agent, logger, cfg)
