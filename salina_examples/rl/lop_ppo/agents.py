...
class LOPPPOMLPPhiAgent(TAgent):
    def __init__(self, **args):
        super().__init__()
        env = instantiate_class(args["env"])
        input_size = env.observation_space.shape[0]
    ...
    def forward(self, t, **args):
        ...
        self.set((self.get_name(),t),value)

class LOPPPOMLPActionAgent(TAgent):
    def __init__(self, **args):
        super().__init__()
        env = instantiate_class(args["env"])
    ...
    def forward(self, t, replay, action_variance, **args):
        ...
        # begin modified PPO for LOP

        u_dist = Uniform(0,1)
        z = u_dist.sample(sample_shape=torch.Size([input.shape[0],1]))
        print("z" + str(z.shape))
        phi_out = z*(self.phi1(input)) + (torch.ones_like(input) - z)*self.phi2(input)

        # end modified PPO for LOP

        mean = torch.tanh(phi_out)
        var = torch.ones_like(mean) * action_variance + 0.000001
        dist = Normal(mean, var)
        self.set(("entropy", t), dist.entropy())
...
class LOPPPOMLPCriticAgent(TAgent):
    def __init__(self, **args):
        super().__init__()
        env = instantiate_class(args["env"])
    ...
    def forward(self, t, **args):
        ...
        print('z: '+ str(z.shape))
        t2 = torch.cat((input,z), dim=1)
        print('tz ' + str(t2.shape))
        critic = self.model_critic(torch.cat((input,z), dim=1)).squeeze(-1)
